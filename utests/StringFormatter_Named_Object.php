...
class StringFormatter_Named_Object extends UnitTestCase {
    public function __construct () {
        parent::__construct ('Mode: named, case: object');
    ...
    public function testCallNonExistentMember () {
        $sf = new StringFormatter ('test: "{val1->test}"');
        $this->assertEqual ($sf->parseNamed (array ('val1' => $this->obj)), 'test: "{val1->test}"');
    }

    public function testCallProperty () {
        $sf = new StringFormatter ('test: "{val1->test_value}"');
        $this->assertEqual ($sf->parseNamed (array ('val1' => $this->obj)), 'test: "'.$this->obj->test_value.'"');
    }

    public function testCallMethod () {
        $sf = new StringFormatter ('test: "{val1->test_method}"');
        $this->assertEqual ($sf->parseNamed (array ('val1' => $this->obj)), 'test: "'.$this->obj->test_value.'"');
    }

    public function testCallPropertyStdClass () {
        $this->expectError ('Object of class stdClass could not be converted to string');
        $sf = new StringFormatter ('test: "{val1->test_value2}"');
        $this->expectError ('Object of class stdClass to string conversion');
        $this->assertEqual ($sf->parseNamed (array ('val1' => $this->obj)), 'test: "Object"');
    }

    public function testCallPropertyClassWithToString () {
        $sf = new StringFormatter ('test: "{val1->test_value3}"');
        $this->assertEqual ($sf->parseNamed (array ('val1' => $this->obj)), 'test: "[StringFormatterTestObject2]"');
    }

    public function testCallMethodStdClass () {
        $this->expectError ('Object of class stdClass could not be converted to string');
        $sf = new StringFormatter ('test: "{val1->test_method2}"');
        $this->expectError ('Object of class stdClass to string conversion');
        $this->assertEqual ($sf->parseNamed (array ('val1' => $this->obj)), 'test: "Object"');
    }

    public function testCallMethodClassWithToString () {
        $sf = new StringFormatter ('test: "{val1->test_method3}"');
        $this->assertEqual ($sf->parseNamed (array ('val1' => $this->obj)), 'test: "[StringFormatterTestObject2]"');
    }

    public function testCallMethodMagicDoesNotExists () {
        $sf = new StringFormatter ('test: "{val1->test_method4}"');
        $this->assertEqual ($sf->parseNamed (array ('val1' => $this->obj2)), 'test: "unknown method: test_method4"');
    }

    public function testCallMethodMagicExists () {
        $sf = new StringFormatter ('test: "{val1->test_method_magic}"');
        $this->assertEqual ($sf->parseNamed (array ('val1' => $this->obj2)), 'test: "test_method_magic"');
    }

    public function testCallPropertyMagicDoesNotExists () {
        $sf = new StringFormatter ('test: "{val1->test_property}"');
        $this->assertEqual ($sf->parseNamed (array ('val1' => $this->obj3)), 'test: "unknown property: test_property"');
    }

    public function testCallPropertyMagicExists () {
        $sf = new StringFormatter ('test: "{val1->test_property_magic}"');
        $this->assertEqual ($sf->parseNamed (array ('val1' => $this->obj3)), 'test: "test_property_magic"');
    }
